...
from py2store import filt_iter, wrap_kvs, ZipReader
...
def mk_dacc(
    ...
    assert zip_path is not None, 'You need to specify where your data is located.'
    assert (
        annots_path is not None
    ), 'You need to specify the file storing your annotations.'
    return Dacc(zip_path, annots_path, wrangle_func, extension, key_path)


class Dacc:
    def __init__(self, zip_path, annots_path, wrangle_func, extension, key_path):
        self.zip_path = zip_path
        self.s = ZipReader(self.zip_path)
        self.annots_df = None
        self.wfs = None
...
_meta = dict(name='local_kgl', description='Local kaggle data set', mk_dacc=mk_dacc)
